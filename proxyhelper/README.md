...
- **Scoped, isolated NAT** — both the DNAT (PREROUTING) and the scoped MASQUERADE (POSTROUTING, `-d host --dport port`, not global) live in dedicated nat chains (`FRIEREN_PROXY_PRE` / `FRIEREN_PROXY`). Disabling routing flushes and removes both chains, so the module's rules are always fully cleaned up — even if the proxy host/port changed between enable and disable (no orphan DNAT or MASQUERADE rules)
...
```
Client → [PREROUTING → FRIEREN_PROXY_PRE: DNAT → proxy:8080] → [IP Forward] → [POSTROUTING → FRIEREN_PROXY: MASQUERADE -d proxy --dport 8080] → Proxy
```
