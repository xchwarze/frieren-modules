...
# Regenerate fstab from detected blocks; bail if it fails (a half-written fstab
# can break mounts on the next boot).
if ! block detect | uci import fstab; then
    echo "[!] Failed to import fstab from block detect."
    exit 1
fi
sleep 2
...
if ! uci commit fstab; then
    echo "[!] Failed to commit fstab config."
    exit 1
fi
sleep 4
...
if [ ! -f "$HOTPLUG_TARGET" ]; then
    if cp "$HOTPLUG_SOURCE" "$HOTPLUG_TARGET"; then
        echo "[+] Copied 20-sd-loader to /etc/hotplug.d/block"
    else
        echo "[!] Failed to copy 20-sd-loader hotplug script."
        exit 1
    fi
fi
